 ...
 // print2_beacon(beacon);
  ...
  if (! known)  // AP is NEW, copy MAC to array and return it
  {
    ...
    if ((unsigned int) aps_known_count >=
      ...
      if (doprint) Serial.printf("exceeded max aps_known\n");
      aps_known_count = 0;
    }
  }
  return known;
...
void displayBeacon(beaconinfo beacon){

#ifdef LCD
  String stringMOne =  String("MAC:[ ");  
  for (int i = 0; i < 6; i++)  {
    stringMOne =  String(stringMOne + String(beacon.bssid[i],HEX));  
    if (i < 5) {
  ...
  if (lcdline == 1) {
    ...
    heltecDisplayMsg(stringMThree,0,lcdline,true,false);

  }else if (lcdline > 1 && lcdline < maxlcdline) {

     heltecDisplayMsg(stringMThree,0,lcdline,false,false);

  } else if (lcdline >= maxlcdline ) {
    heltecDisplayMsg(stringMThree,0,lcdline,false,false);
    lcdline=0;
  }
  lcdline++;
...
void print_beacon(beaconinfo beacon)
{
  ...
  } else if (beacon.err == -4)
    {
    counter++;
    if (doprint) {
      Serial.printf("BEACON: <===(UNASSOC.)== [%32s]  ", beacon.ssid);
      for (int i = 0; i < 6; i++) Serial.printf("%02x:", beacon.bssid[i]);
      Serial.printf("   %2d", beacon.channel);
      Serial.printf("   %4d\r\n", beacon.rssi);
    }
...
void promisc_cb(uint8_t *buf, uint16_t len)
{
  ...
  if (len == 12) {
    struct RxControl *sniffer = (struct RxControl*) buf;
     ...
     Serial.printf("C:");
     for (int i = 0; i < 105; i++) Serial.printf("%02x", buf[i]);
     Serial.printf("\n");
  } else if (len == 128) {
    //Attached and Unattached clients are both len=128 
